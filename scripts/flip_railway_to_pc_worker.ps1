...
#
# Sets on the ChalybClip service (triggers a redeploy):
...
if (-not $token) { Write-Error "CHALYBCLIP_MODAL_TOKEN missing on the ChalybClip service" }

railway variables --service ChalybClip `
...
Write-Output "Variables set. Trigger the deploy when ready: railway redeploy --service ChalybClip"
